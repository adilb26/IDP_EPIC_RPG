...
def rpg_hunt(level, user):
    ...
    defense_reduction = user.get('defense', 0)
    hp_loss = max(0, hp_loss - defense_reduction)
    
    attack_bonus = user.get('attack', 0) // 5
    xp_gain += attack_bonus
    ...
    luck_roll = random.randint(1, 100) + user.get('luck', 0)
    rare_items = ["Legendary Gem", "Enchanted Scroll", "Dragon Scale", "Mystic Orb"]
    if luck_roll >= 95:
        rare_item = random.choice(rare_items)
        if rare_item in user['inventory']:
            user['inventory'][rare_item] += 1
        else:
            user['inventory'][rare_item] = 1
    ...
    if luck_roll >= 95:
        print(f"✨ RARE DROP! Found: {rare_item}")
